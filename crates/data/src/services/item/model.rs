...
use shroom_pkt::{CondOption, ShroomExpirationTime, ShroomTime};
    ...
    fn from(value: equip_item::Model) -> Self {
        ...
        Self {
            info: ItemInfo {
                ...
                cash_id: value.cash_id.map(|i| i as u64 + 1),
...
impl From<&EquipItem> for proto_item::EquipItemInfo {
    fn from(value: &EquipItem) -> Self {
        // TODO handle either cash id or sn properly
        let sn: CondOption<u64> = if value.cash_id.is_some() {
            None.into()
        } else {
            Some(value.db_id.unwrap_or(thread_rng().gen()) as u64).into()
        };
