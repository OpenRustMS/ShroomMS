...
use crate::services::meta::{meta_service::SkillMeta, schema_base::EitherStringOrInt};
...
impl SkillData {
    fn eval_helper(&self, v: &Option<EitherStringOrInt>) -> Option<i64> {
        v.as_ref().map(|cost| cost.eval(self.level as i64).unwrap())
    }

    pub fn max_level(&self) -> usize {
        self.meta.max_level.unwrap_or(0) as usize
    }
    ...
    pub fn mp_cost(&self) -> Option<usize> {
        self.eval_helper(&self.meta.mp_cost).map(|n| n as usize)
    }
...
impl From<(SkillId, SkillMeta)> for SkillData {
    fn from((id, skill): (SkillId, SkillMeta)) -> Self {
        SkillData {
            id,
            level: 0,
            mastery_level: skill.mastery_level.map(|n| n as usize),
            expires_at: None,
            cooldown: None,
            meta: skill,
        }
...
impl SkillSet {
    ...
    pub fn set_cooldown(&mut self, skill_id: SkillId, dur: Duration) {
        if let Some(cd) = self.skill_cooldowns.get_mut(&skill_id) {
            *cd = dbg!(Cooldown::from_duration(dur));
        } else {
            self.skill_cooldowns
                .insert(skill_id, dbg!(Cooldown::from_duration(dur)));
        }
