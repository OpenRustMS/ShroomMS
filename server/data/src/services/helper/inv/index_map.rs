...
impl<Id: Eq + std::hash::Hash + Copy + Clone> IdIndexMap<Id> {
    ...
    pub fn slots_for_id(&self, id: &Id) -> Option<&[u16]> {
        self.0.get(id).map(|s| s.as_slice())
    }

    pub fn slots_iter_for_id(&self, id: Id) -> impl Iterator<Item = usize> + '_ {
        self.slots_for_id(&id)
            .into_iter()
            .flatten()
            .map(|i| *i as usize)
    }
...
mod tests {
    ...
    fn index_map() {
        ...
        // Assert ids
        assert_eq!(ix.slots_for_id(&0), Some([0, 1].as_slice()));
        assert_eq!(ix.slots_for_id(&1), Some([2].as_slice()));
        ...
        // Assert ids
        assert_eq!(ix.slots_for_id(&0), Some([2, 1].as_slice()));
        assert_eq!(ix.slots_for_id(&1), Some([0].as_slice()));

        // Remove an item
        ix.remove(0, 2);
        assert_eq!(ix.slots_for_id(&0), Some([1].as_slice()));
    ...
    fn sorted_slots() {
        ...
        assert_eq!(ix.slots_for_id(&0), Some([1, 2, 3, 4].as_slice()));
    }
}
