...
use futures::Future;
use proto95::{
    game::script::{AskMsg, AskNumberMsg, AskTextMsg, MsgParamFlags, SayMsg, ScriptMessage, ScriptAnswerReq, OptionAnswer},
    id::{ItemId, SkillId}, fmt::{SkillName, SkillIcon},
};
use tokio::sync::mpsc;
...
impl From<ScriptAnswerReq> for NpcAction {
    fn from(req: ScriptAnswerReq) -> Self {
        match req {
            ...
            ScriptAnswerReq::InputText(v) if v.is_some() => Self::InputTxt(v.as_ref().unwrap().clone()),
            ScriptAnswerReq::InputNumber(v) => Self::InputNum(v),
            ScriptAnswerReq::InputSelection(v) if v.is_some() => NpcAction::Selection(v.unwrap() as usize),
            ScriptAnswerReq::AvatarSelection(v) if v.is_some() => NpcAction::AvatarSelection(v.unwrap() as usize),
            ScriptAnswerReq::AvatarMembershipSelection(v) if v.is_some() => NpcAction::AvatarSelection(v.unwrap() as usize),
            ScriptAnswerReq::PetSelection(v) if v.is_some() => NpcAction::PetSelection(v.unwrap() as usize),
            ScriptAnswerReq::InputBoxText(v)  if v.is_some() => Self::InputTxt(v.as_ref().unwrap().clone()),
